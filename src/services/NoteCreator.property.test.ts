  ...
  it("uploadDate 전달 시 파일명이 업로드 날짜 YY-MM-DD 접두사로 시작하고 특수 문자를 포함하지 않는다", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(titleArbitrary, isoDateArbitrary, async (title, uploadDate) => {
        ...
        // 속성 5: 제목 부분(날짜 접두사 "YY-MM-DD " 이후, ".md" 이전)이 200바이트를 넘지 않는다
        const titlePart = fileName.slice("YY-MM-DD ".length).replace(/\.md$/, "");
        expect(new TextEncoder().encode(titlePart).length).toBeLessThanOrEqual(200);
      }),
